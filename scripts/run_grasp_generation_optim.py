"""
Stage 0 – Grasp Generation + Isaac Refinement (Optimization version)
=====================================================================
...
Usage:
    /isaac-sim/python.sh scripts/run_grasp_generation_optim.py
    /isaac-sim/python.sh scripts/run_grasp_generation_optim.py --shapes cube --num_sizes 1
"""
...
def _expand_22dof_to_24dof(joint_angles_22: np.ndarray) -> np.ndarray:
    """
    Convert DexGraspNet 22-DOF joint angles to Isaac Sim 24-DOF format.

    DexGraspNet Shadow Hand MJCF (22 DOF, no wrist):
      [0-3]   FF: FFJ4, FFJ3, FFJ2, FFJ1
      [4-7]   MF: MFJ4, MFJ3, MFJ2, MFJ1
      [8-11]  RF: RFJ4, RFJ3, RFJ2, RFJ1
      [12-16] LF: LFJ5, LFJ4, LFJ3, LFJ2, LFJ1
      [17-21] TH: THJ5, THJ4, THJ3, THJ2, THJ1

    Isaac Sim Shadow Hand USD (24 DOF):
      [0-1]   WR: WRJ1, WRJ0          (wrist, set to 0)
      [2-5]   FF: FFJ4, FFJ3, FFJ2, FFJ1
      [6-9]   MF: MFJ4, MFJ3, MFJ2, MFJ1
      [10-13] RF: RFJ4, RFJ3, RFJ2, RFJ1
      [14-18] LF: LFJ5, LFJ4, LFJ3, LFJ2, LFJ1
      [19-23] TH: THJ4, THJ3, THJ2, THJ1, THJ0  (NO THJ5, has THJ0)

    Key difference: DexGraspNet has THJ5 (thumb base rotation), Isaac Sim
    does NOT have THJ5 but has THJ0 (thumb tip flexion) instead.
    """
    q24 = np.zeros(24, dtype=np.float32)
    # FF/MF/RF/LF: indices [0:17] → [2:19] (identical mapping)
    q24[2:19] = joint_angles_22[:17]
    # Thumb: skip THJ5 (index 17, not in Isaac Sim USD)
    # Map THJ4-THJ1 (indices 18-21) → Isaac Sim slots [19:23]
    q24[19:23] = joint_angles_22[18:22]
    # THJ0 (index 23) stays at 0 (not present in DexGraspNet)
    return q24
...
def generate_grasps(spec, args, num_fingers, num_dof, hand_name, device):
    ...
    # 22 DOF → 24 DOF: properly remap thumb joints (THJ5 skipped, THJ0 zeroed)
    for g in grasp_set.grasps:
        if g.joint_angles is not None and len(g.joint_angles) == 22:
            g.joint_angles = _expand_22dof_to_24dof(g.joint_angles)
